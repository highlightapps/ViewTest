...
import bluetooth.BluetoothUuid;
...
public class BluetoothService extends Service {
    ...
    public void onCreate() {
        super.onCreate();
        Log.d("BluetoothService", getClass().getSimpleName() + " onCreate ");
    ...
    public IBinder onBind(Intent intent) {
        Log.d("BluetoothService", getClass().getSimpleName() + " onBind ");
        return new BluetoothServiceBinder();
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("BluetoothService", getClass().getSimpleName() + " onStartCommand ");
    ...
    public void onDestroy() {
        if (sBluetoothSocket != null && sBluetoothSocket.isConnected()) {
            try {
                ...
                Log.d("BluetoothService","in onDestroy, sBluetoothSocket.close failed !");
                e.printStackTrace();
            }
        }
        super.onDestroy();
    ...
    public void getPhoneBook() {
        if (sPbapClient != null && sPbapClient.getState() == BluetoothPbapClient.ConnectionState.CONNECTED) {
            Log.d("BluetoothService","pulling the PhoneBook, it may take a long time ! ");
            sPbapClient.pullPhoneBook(BluetoothPbapClient.PB_PATH);
        } else {
            Log.d("BluetoothService","-----------------------------");
            Log.d("BluetoothService","sPbapClient is not ready ! ");
          //  Log.d("BluetoothService",sPbapClient);
          //  Log.d("BluetoothService",sPbapClient.getState());
            Log.d("BluetoothService","-----------------------------");
    ...
    public void establishPbap(String address) {
        ...
        if (sPbapClient == null) {
            ...
            sPbapClient.connect();
            Log.d("BluetoothService","after sPbapClient.connect()");
        }
    }
    ...
    public void showAllAvailableUuids(BluetoothDevice device) {
        ...
        if (uuids == null || uuids.length == 0) {
            Log.d("BluetoothService","get uuids failed");
        } else {
            for (ParcelUuid uuid : uuids) {
                Log.d("BluetoothService","uuid is : " + uuid);
            }
            Log.d("BluetoothService","-------------");
        }
    }
    ...
    public void showBondedState(BluetoothDevice device){
        ...
        switch (device.getBondState()) {
            case BluetoothDevice.BOND_BONDED: {
                Log.d("BluetoothService","BONDED");
            }
            break;
            case BluetoothDevice.BOND_NONE: {
                Log.d("BluetoothService","NONE");
            }
            break;
            case BluetoothDevice.BOND_BONDING: {
                Log.d("BluetoothService","BONDING");
            }
            break;
            default: {
            }
    ...
    public void showDiscoveryState(){
        if (sBluetoothAdapter.isDiscovering()) {
            Log.d("BluetoothService","@@@ it is discovering @@@ ");
            sBluetoothAdapter.cancelDiscovery();
        } else {
            Log.d("BluetoothService","@@@ it is not discovering @@@");
        }
    }
    ...
    public void establishSocket(String address) {
        ...
        try {
            Log.d("BluetoothService","try to createRfcommSocketToServiceRecord");
            sBluetoothAdapter.cancelDiscovery();
//            sBluetoothSocket = device.createRfcommSocketToServiceRecord(uuid.getUuid());
            ...
            try {
                ...
                Log.d("BluetoothService","sBLuetoothSocket.connect() is failed -------from the IOException");
                try {
                    sBluetoothSocket.close();
            ...
            Log.d("BluetoothService","state of sBluetoothSocket : " + sBluetoothSocket.isConnected());


        } catch (IOException e) {
            Log.d("BluetoothService","createRfcommSocketToServiceRecord failed");
            e.printStackTrace();
        }
    ...
    public void getHeadsetClient() {
        BluetoothProfile.ServiceListener listener = new BluetoothProfile.ServiceListener() {
            ...
            public void onServiceConnected(int profile, BluetoothProfile proxy) {
                Log.d("BluetoothService","BluetoothProfile.ServiceListener onServiceConnected");
                if (profile == BluetoothProfile.HEADSET) {
                    if (proxy == null) {
                        Log.d("BluetoothService","the proxy is null ! ");
                        return;
                    } else {
                        Log.d("BluetoothService","the proxy is not null ! ");
                    }
                    ...
                    if (connectedList.size() == 0) {
                        Log.d("BluetoothService","the connectedList is of size 0 ! ");
                        return;
                    }
                    ...
                    Log.d("BluetoothService","isAudioConnected : " + isAudioConnected);

                    boolean isVoiceRecognitionEnable = headset.startVoiceRecognition(device);
                    Log.d("BluetoothService","isVoiceRecognitionEnable : " + isVoiceRecognitionEnable);
            ...
            public void onServiceDisconnected(int profile) {
                Log.d("BluetoothService","onServiceDisconnected");
            }
        };

        if (sBluetoothAdapter.getProfileProxy(sContext, listener, BluetoothProfile.HEADSET)) {
            Log.d("BluetoothService","Get the profile proxy object associated with the profile --- success ! ");
        } else {
            Log.d("BluetoothService","getProfileProxy failed ! ");
        }
    }
    ...
    public static class BluetoothServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            Log.d("BluetoothService",msg.toString());
            switch (msg.what) {
                case BluetoothPbapClient.EVENT_PULL_PHONE_BOOK_DONE: {
                    Log.d("BluetoothService","EVENT_PULL_PHONE_BOOK_DONE");
                    sPbapClient.disconnect();
                }
                break;
                case BluetoothPbapClient.EVENT_SESSION_CONNECTED: {
                    Log.d("BluetoothService","EVENT_SESSION_CONNECTED");
                }
                break;
                case BluetoothPbapClient.EVENT_SESSION_DISCONNECTED: {
                    Log.d("BluetoothService","EVENT_SESSION_DISCONNECTED");
                }
                break;
                default: {
                }
